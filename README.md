...
화면 중앙의 원형 버튼 하나로 연주하는 웹앱. (구 이름: 연주 웹앱)

- 터치 = 저장된 멜로디를 순서대로 한 음씩 재생
- 누른 채 위/아래로 드래그 = 음 높낮이 변화 (원래 음에서 최대 온음 정도까지만)
- 누른 채 좌우로 흔들기 = 비브라토

### 화음 스타일 (Harmony Style)

멜로디 아래에 스타일별로 다르게 보이싱한 화음 반주를 자동으로 깔아줍니다. 감지된 조성과
각 음의 스케일 디그리를 바탕으로 진행마다 그 자리의 정격 화음을 만들고, 스타일에 따라
음색·보이싱·밀도를 다르게 배치합니다.

- **찬송가**: SATB 코랄식. 멜로디가 소프라노, 화음 신스가 알토·테너·베이스를 촘촘한 근접
  포지션으로 채워 정적이고 예측 가능한(I-IV-V-I 중심) 텍스처를 만듭니다.
- **가스펠**: 7th/9th 확장 화음, 베이스가 근음↔5음을 오가는 워킹 베이스, 짝수 박에 살짝
  당겨지는(싱커페이션) 어택 타이밍.
- **CCM/워십**: sus4·add9 위주로 근음·중간·상성부를 넓게 벌린 오픈 보이싱 + 느린 어택의
  패드 신스로 공간감을 냅니다.
- **CCLI 밴드**: 7th 화음을 레이어링하되, 곡 진행률(초반/중반/후반)에 따라 겹쳐지는 성부
  수를 늘려 절→후렴 같은 다이나믹 빌드업을 흉내냅니다.

화음 없이 멜로디만 듣고 싶으면 "화음 없음"을 선택하세요. 구현은 `src/lib/harmony.ts`
(화성 이론·보이싱 규칙)와 `src/audio/player.ts`(재생 시 화음 신스 트리거)에 있습니다.
...
`music.jungsim.org` 접속 후 브라우저 메뉴에서 "홈 화면에 추가"(또는 "앱 설치")를
하면 아이콘이 생기고 주소창 없이 전체화면 앱처럼 실행됩니다. 서비스워커는 캐싱을
하지 않고 오프라인일 때 안내 화면만 보여주므로, 실행할 때마다 항상 서버의 최신
화면을 띄웁니다. (`public/pwa/manifest.webmanifest`, `public/sw.js`)
